...
- 当前画布项目和“我的素材”主要保存在浏览器本地，不要在文档中误写成已支持云同步。自建后端接口约定见 `docs/content/docs/development/backend-api.zh-CN.mdx`；接口未接通前不要写“资源已全部走服务端”。
- 当前 AI API Key 存在浏览器本地，并由前端直接请求 OpenAI 兼容接口；涉及安全说明时要写清楚。
- 产品已移除右侧 Agent 面板及相关前端入口，不要再加回连接 Codex / Canvas Agent 的 UI。
