...
import { APP_ID, APP_STORAGE_DB } from "@/constant/app-id";
...
const imageLogStore = localforage.createInstance({ name: APP_STORAGE_DB, storeName: "image_generation_logs" });
const videoLogStore = localforage.createInstance({ name: APP_STORAGE_DB, storeName: "video_generation_logs" });
...
async function syncDomain<T>(config: WebdavSyncConfig, onProgress: AppSyncProgress | undefined, options: SyncDomainOptions<T>): Promise<SyncDomainResult<T>> {
    try {
        ...
        const manifest: DomainManifest<T> = { app: APP_ID, version: 1, domain: options.key, exportedAt: new Date().toISOString(), data: mergedData, files: uploaded.files };
...
async function readDomainManifest<T>(config: WebdavSyncConfig, domain: DomainKey, emptyData: T): Promise<DomainManifest<T> | null> {
    ...
    if (data.app !== APP_ID || data.domain !== domain) throw new Error(i18n.t("config.webdav.errors.invalidManifest", { domain }));
    return {
        app: APP_ID,
        version: 1,
        domain,
