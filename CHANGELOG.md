...
+ [新增] 可选接入 Supabase：配置后支持邮箱登录/注册，并按用户隔离同步画布项目；未配置时仍为纯本地模式。
+ [调整] 火山方舟 / Seedance 视频时长支持选择 30 秒。
+ [调整] 新建视频默认使用当前视频模型，分辨率 480p、比例自适应、时长 30 秒。
+ [调整] 新建视频默认使用当前 Seedance 2.5 模型、480p、自适应比例和 30 秒时长。
+ [调整] 移除右侧 Agent 面板及网页内 Codex 连接入口。
+ [调整] 项目更名为 WhatsLove Creators，Git 主远程改为 AWS CodeCommit；浏览器本地存储命名空间同步切换为 `whatslove-creators`（旧数据不保留）。
+ [调整] 移除顶部 GitHub 外链与上游版本在线检查，版本说明仅读取当前构建内置 CHANGELOG。
